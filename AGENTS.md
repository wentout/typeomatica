...
- **2026-09-03 decision:** the `super()`-returns-`new Proxy(this, handler)` machinery — which WOULD make derived class field initializers hit the `defineProperty` trap, multi-level chains included — is deliberately **not adopted**. Evidence lives in `examples/16-defineproperty-trap-field-initializers.js`. Rationale, decisive first: **the bypass IS the opt-out** — if initializers were intercepted, strictness would be mandatory with no escape; as it stands the user chooses per field (initializer = unrestricted, constructor-body assignment = checked). Explicit `Object.defineProperty(this, …)` — in the constructor or later on the finished instance — is the second hatch (defines directly on the receiver, no chain walk); a trap would close both hatches at once. Then: complexity; spec-level blindness to private members (and privates on a proxy-returning base throw on later access); the will to keep class fields unrestricted during construction. The original (pre-probe) reason stands too: down-level targets (ES3-era TS) rewrite fields into constructor assignments, so interception is not a stable concept across pipelines — strictness is meant for explicit `[[Set]]` while the constructor runs. The exposed API may vary after construction; construction stays plain. Do not reintroduce without re-opening that decision.
